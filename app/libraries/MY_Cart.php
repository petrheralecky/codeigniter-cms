...
class MY_Cart extends CI_Cart {
	...
	//                    ADDED
		if((!empty($items['options']) && is_array($items['options']))||(!empty($items['price']) && is_numeric($items['price']))){
			$this->_cart_contents[$items['rowid']]['options'] = $items['options'];
			$c = $this->_cart_contents[$items['rowid']];
			if(!empty($items['options']) && is_array($items['options'])){
				$c['options'] = $items['options'];
			}
			if(!empty($items['price']) && is_numeric($items['price'])){
				$c['price'] = $items['price'];
			}
			if(isset($items['qty']) && is_numeric($items['qty'])){
				$c['qty'] = $items['qty'];
			}
			unset($c['rowid']);
			unset($this->_cart_contents[$items['rowid']]);
