...
function control_input(name,form_name,only_error,ajax_path,tagtyp){
	...
	if(!only_error || element.className.indexOf(' not_valid') != -1){
		processAjax(form_name,"value="+value+"&name="+name+"&citac="+citac_odeslanych,ajax_path); //treti parametr.... +others
		citac_odeslanych++;
	}
}
...
function dt2human (){
	...
	for(var i=0;i<e.length;i++){
		if (re1.exec(e[i].value)) e[i].value = (parseInt(RegExp.$3)+"."+parseInt(RegExp.$2)+"."+RegExp.$1+" "+RegExp.$4+":"+RegExp.$5+":"+RegExp.$6);
		if (re2.exec(e[i].value)) e[i].value = (parseInt(RegExp.$3)+"."+parseInt(RegExp.$2)+"."+RegExp.$1);
		if (re3.exec(e[i].value)) e[i].value = (parseInt(RegExp.$3)+"."+parseInt(RegExp.$2)+"."+RegExp.$1+" "+RegExp.$4+":"+RegExp.$5);
	}
}
